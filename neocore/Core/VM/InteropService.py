...
from neocore.Core.Contract import StackItemType
#logger = log_manager.getLogger('vm')
from neocore.Core.VM.Mixins import EquatableMixin
...
class StackItem(EquatableMixin):
    ...
    def GetArray(self):
        raise Exception('Error: trying to get array:: %s " % self')
...
class Array(StackItem, CollectionMixin):
    ...
    def GetBigInteger(self):
        raise Exception("Error: Trying to get big integer %s " % self)

    def GetBoolean(self):
        return len(self._array) > 0

    def GetByteArray(self):
        raise Exception("Error: Trying to get bytearray integer %s " % self)
