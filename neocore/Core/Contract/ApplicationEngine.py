...
from neocore.Core.VM.ExecutionContext import ExecutionContext
from neocore.Core.VM.InteropService import CollectionMixin, Map, Array
from neocore.Cryptography.Crypto import Crypto
from neocore.Fixed8 import Fixed8

# used for ApplicationEngine.Run
from neocore.Core.State.ContractState import ContractPropertyState
from neocore.Core.VM.InteropService import Array
    ...
    def CheckInvocationStack(self):
        ...
        if opcode == CALL or opcode == APPCALL:
            if self.InvocationStack.Count >= maxInvocationStackSize:
                raise Exception("Error: INVOCATION STACK TOO BIG, RETURN FALSE")
    ...
    def CheckItemSize(self):
        ...
        if opcode == PUSHDATA4:
            ...
            if length > self.maxItemSize:
                raise Exception("Error: ITEM IS GREATER THAN MAX ITEM SIZE!")
            ...
            if cx.EvaluationStack.Count < 2:
                raise Exception("NOT ENOUGH ITEMS TO CONCAT")
            ...
            try:
                ...
                raise Exception("COULD NOT GET STR LENGTH!")

            if length > self.maxItemSize:
                raise Exception("ITEM IS GREATER THAN MAX SIZE!!!")
                return False
