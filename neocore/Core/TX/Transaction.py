...
Usage:
    from neocore.Core.Transaction import Transaction
"""
import sys
...
from neocore.Core.InventoryType import InventoryType
from neocore.Core.Mixins import InventoryMixin
from neocore.Cryptography.Crypto import Crypto
from neocore.IO.Mixins import SerializableMixin
from neocore.IO.MemoryStream import StreamManager
from neocore.IO.BinaryReader import BinaryReader
from neocore.Core.Mixins import EquatableMixin
...
class Transaction(InventoryMixin):
    ...
    def __init__(self, inputs=[], outputs=[], attributes=[], scripts=[]):
        ...
        Create an instance.
        Args:
            inputs (list): of neocore.Core.CoinReference.CoinReference.
            outputs (list): of neocore.Core.TX.Transaction.TransactionOutput items.
            attributes (list): of neocore.Core.TX.TransactionAttribute.
            scripts:
        """
    ...
    def References(self):
        ...
        if self.__references is None:
            ...
            for hash, group in groupby(self.inputs, lambda x: x.PrevHash):
                ...
                tx, height = Blockchain.GetBlockchain(self).GetTransaction(hash.ToBytes())
                if tx is not None:
                    for input in group:
    ...
    def NetworkFee(self):
        ...
        if self._network_fee is None:
            ...
            for coin_ref in self.References.values():
                from neocore.Core.Blockchain import Blockchain
                if coin_ref.AssetId == Blockchain.GetBlockchain(self).SystemCoin().Hash:
            ...
            for tx_output in self.outputs:
                from neocore.Core.Blockchain import Blockchain
                if tx_output.AssetId == Blockchain.GetBlockchain(self).SystemCoin().Hash:
    ...
    def DeserializeFrom(reader):
        ...
        from neocore.Core.TX.PublishTransaction import PublishTransaction
        from neocore.Core.TX.InvocationTransaction import InvocationTransaction
        from neocore.Core.TX.EnrollmentTransaction import EnrollmentTransaction
        from neocore.Core.TX.StateTransaction import StateTransaction
    ...
    def DeserializeUnsignedWithoutType(self, reader):
        ...
        self.Attributes = reader.ReadSerializableArray('neocore.Core.TX.TransactionAttribute.TransactionAttribute',
                                                       max=self.MAX_TX_ATTRIBUTES)
        self.inputs = reader.ReadSerializableArray('neocore.Core.CoinReference.CoinReference')
        self.outputs = reader.ReadSerializableArray('neocore.Core.TX.Transaction.TransactionOutput')
    ...
    def GetScriptHashesForVerifying(self):
        ...
        for key, group in groupby(self.outputs, lambda p: p.AssetId):
            from neocore.Core.Blockchain import Blockchain
            asset = Blockchain.GetBlockchain().GetAssetState(key.ToBytes())
            if asset is None:
