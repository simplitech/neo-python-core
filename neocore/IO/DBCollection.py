...
class DBCollection:
    ...
    def __init__(self, dbService, prefix, class_ref):
        self.DB = dbService
        self.Prefix = prefix
    ...
    def _BuildCollectionKeys(self):
        for key in self.DB.getDbIteratorForPrefix(prefix=self.Prefix, include_value=False):
            key = key[1:]
    ...
    def Find(self, key_prefix):
        ...
        res = {}
        for key, val in self.DB.getDbIteratorForPrefix(prefix=key_prefix):
